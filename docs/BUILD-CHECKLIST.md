...
- [ ] **Upstash Kafka:** Topic `jobs` exists; bootstrap URL + credentials in `.env`; a minimal producer can publish a test message to `jobs` and succeed.
- [ ] **Upstash Redis:** Connection string in `.env`; client can `PING` or set/get a test key.
